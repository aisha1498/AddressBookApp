...
const createInnerHtml = () => {
    const headerHtml = "<th>Fullname</th><th>Gender</th><th>profession</th><th>EmailID</th><th>Address</th><th>City</th><th>State</th><th>Zip Code</th><th>Phone Number</th>"
    let innerHtml = `${headerHtml}`;
    for (const addressBook of addressBookList) {
        innerHtml = `${innerHtml}
    <tr>
        <td>${addressBook._name}</td>
        <td>${addressBook._gender}</td>
